  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ParkingAppScope.of(context).addHistory(
        HistoryItem(mode: ModeType.exit, plateText: _plateController.text.trim().toUpperCase(), result: result),
      );
      Navigator.of(context).pushNamed(ResultScreen.routeName, arguments: result);
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No se pudo enviar la solicitud de salida.')),
      );
    } finally {
      if (mounted) {
